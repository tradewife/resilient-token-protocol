...
- `BUILD_PLAN_v3.md` — active remediation schedule
...
## Current Build State (as of 2026-04-08)

### Completed
- ✅ All CRITICAL findings fixed (C-1, C-2, C-3)
- ✅ All HIGH findings fixed (H-1 through H-5)
- ✅ Most MEDIUM findings fixed (M-1, M-3, M-4, M-5)
- ✅ Rust swarm: 88/88 tests passing
- ✅ Treasury: compiles clean, BPF `.so` produced
- ✅ Anchor integration tests: 15 passing
- ✅ Night shift CI pipeline: unblocked and running (cron 14:00 UTC / midnight AEST)
- ✅ ONBOARDING.md updated for Week 3

### In Progress / Stubbed (Week 3 work)
- 🔧 `rtp/swarm/src/wings/trading/mod.rs` — stub, needs `bridge.rs` (Python↔Rust interface)
- 🔧 `rtp/swarm/src/wings/security/mod.rs` — stub, only heartbeat
- 🔧 `rtp/swarm/src/wings/knowledge/mod.rs` — stub, only query handler
- 🔧 `rtp/swarm/src/wings/futureproof/mod.rs` — stub, only heartbeat
- 🔧 Coordinator integration tests (not yet written)
- 🔧 Full end-to-end swarm ↔ treasury message flow (not yet wired)

### Known Outstanding Issues
- M-2 from the security audit (check `docs/SECURITY_AUDIT_2026-04-07.md` for status)
- `stub_review()` in audit wing now rejects EvolveProposal but is still a stub — real 3-agent tribunal logic is implemented but needs wiring to the coordinator flow
...
2. **Anchor treasury program** (`rtp/programs/rtp-treasury/`) — focus on:
   - All previously patched invariants (see "Completed" above) — verify patches are solid
...
## How to Scope to the Last Two Pushes

The two most recent pushes to `main` are:

| Commit | SHA | Summary |
|--------|-----|---------|
| Latest | `475e6d4` | `docs: add SYMPHONY.md — Rust orchestrator implementation spec` |
| Previous | `040be63` | `ci: add git pull --rebase before push; commit 2026-04-08 night results` |

To review only these changes:
```bash
git diff main~2 main
# or
git diff 040be635aa513a38bcec17dd25559a43e2b61f9e 475e6d483d037e397edfb4bd264e2c30e86e6565
```

For a review of the full audit-remediation batch (all fixes since the security audit):
```bash
git diff de35f261686e6227393e43ca92adde4807873214 main
```
...
## Week 3 Readiness
Is the codebase in a state to begin bridge.rs and wing wiring work? What must be resolved first?
```
...
4. `ONBOARDING.md` — current build state and Week 3 plan
5. `BUILD_PLAN_v3.md` — active remediation schedule
...
- **GitHub**: https://github.com/tradewife/resilient-token-protocol
