...
**Last updated:** 2026-08-08 (late, session 4) — **2.69 IS LIVE ON GMTRADE (no paper trade, per user decision).** Deploy `a4379e21` SUCCESS, `Dry run: false`, `[GM] venue ready`, 300s GM-aware watchdog cycle. The Rust trader now executes Survivor 2.69 against GMTrade `SOL/USD[WSOL-USDC]` via a native gmsol-sdk adapter (commit `b130da2`), dispatched by `RTP_TRADER_VENUE=gmtrade` + `RTP_TRADER_DRY_RUN=0` on Railway (rtp-trader service). USDC-collateral sizing: 20% of wallet SOL balance valued at spot, capped at the USDC ATA balance (76.59 available), ×9x leverage. Keeper-fill probe passed same-day (commit `124baf6`): fills in 24.8–31.0s, inside Chainlink band, measured round-trip fee 0.022% = v8e model. Docker fixes landed: git in builder (git dep fetch), rust 1.88→1.94 (ruint 1.20 MSRV), make/perl + OPENSSL_NO_VENDOR (vendored openssl build), runtime bookworm→trixie (GLIBC_2.39). Prior sessions today: keeper-fill probe PASS + GMTrade measured-cost gate suite PASS (v8e 10/10 ×3) + website/Stripe launch (live A$4,500 link). **OPEN:** GST @ Build Day Aug 18; homepage vitals reframe undecided; RB strategy manufacturing (separate job). $10 LONG + $10 SHORT round trips executed via gmsol-sdk `market_increase`/`market_decrease` against SOL/USD[WSOL-USDC] (`3M4vW1…`, store `CTDLvGGXnoxvqLyTpGzdGLg9pD6JexKxKXSV8tqqo8bN`). GMTrade's keepers filled every order with NO client-side oracle access — fill latency 24.8–31.0s, fills inside Chainlink [min,max] band, every round trip ended FLAT with collateral returned. **Measured round-trip order fee 0.022% of notional — exactly matches the v8e model** (0.012% neg-impact + 0.010% pos-impact); price impact on $10/$50K OI ≈ $0.00002. Key adapter facts: two-step keeper model (user signs create_order, keeper executes), no LP deposit needed for trading (market_increase deposits collateral atomically), USDC collateral needs zero setup, `complete_order()` polls TradeEvent for fill detection, unit prices are fixed-point 10^(20−index_decimals)=1e11 (NOT 1e20 like USD amounts), execution fee floor 300k lamports (used 500k). Probe + results at `research/missions/gmtrade_probe/` (commit `124baf6`). **NEXT: build the GMTrade execution adapter** (gmsol-sdk dependency, replace Flash client in Rust trading wing), then wire trader + DRY_RUN off. Prior session-3 work: **WEBSITE LAUNCH + STRIPE LIVE** — production serves the live A$4,500 payment link (`buy.stripe.com/8x2bIU7GH0AFbAQ6qVd7q00`), Dockerfile build-arg fix for NEXT_PUBLIC_ baking (commit `17dd993`), GST deferred to Build Day Aug 18. Full rework of dashboard copy (bespoke-service voice, venue-neutral, GMTrade measured costs as current live venue), merged to `main`, deployed to Railway production — verified serving (hero "A trading engine built for one account: yours.", §1 "The blueprint is running. With real capital."). **Stripe is LIVE**: live product `prod_V28L5w4SBN3hlS`, price `price_1U245H31u3bugCqx2kBt8k22` (A$4,500 AUD one-time), payment link `https://buy.stripe.com/8x2bIU7GH0AFbAQ6qVd7q00` — production serves the live link (verified). **Build fix:** `Dockerfile.dashboard` builder stage now declares `ARG NEXT_PUBLIC_RTP_DIAGNOSTIC_PAY_URL` + `ENV` so Next.js bakes it at build time; env var set on rtp-dashboard Railway service. Two deploys served stale static HTML (`x-nextjs-cache: HIT`) before the fix. Live keys stored in `.secrets/stripe-live-key` / `stripe-live-publishable-key`, objects recorded in `.secrets/stripe-live-resources` (all gitignored). **OPEN ITEMS (Build Day Aug 18):** (a) GST/automatic_tax for AU A$4,500 sale — Managed Payments disabled on link, product tax_code txcd_10000000 set as placeholder; sort with Stripe 1:1. (b) Homepage vitals: keep raw −2.56% PnL or reframe (user undecided). **NEXT: 2.69 live on GMTrade** — keeper-fill probe trades ($10 long+short), account setup, GMTrade execution adapter replacing Flash client in Rust swarm, wire trader, DRY_RUN off. User decision: 2.69 goes LIVE on GMTrade directly, NO paper trade. Client RB strategy manufacturing is a separate later job.

Prior same-session: **GMTrade MEASURED-COST VERIFICATION PASSED (v8e, commit 103a408).** Built gmsol-sdk probe (`venues/gmx-solana/examples/rtp_probe.rs`, copy at `research/missions/gmtrade_probe/`) and read GMTrade mainnet store `Gmso1uvJ…` on-chain: order fees 0.010/0.012% per side CONFIRMED; `skip_borrow_for_smaller_side=TRUE` (minority OI side pays 0 borrow); RB's accumulation market is SOL/USD[WSOL-USDC] `3M4vW1…` (OI $48K/$2K, longs majority → borrow 0.0036%/hr measured, kink-max 0.0114%/hr; funding cap 0.0856%/day; liq fee 0.05%). Gate suite on measured costs: **ALL THREE borrow scenarios pass 10/10** — E1 measured-now +57.7% (3.53 SOL @5x), E2 stress-kink +52.8% (3.30 SOL @5x), E3 low-usage +58.7% (3.57 SOL @5x), all dd <20%, sensitivity ROBUST 0 flips. Institutional gate cleared — config validated on the venue's LIVE costs. Remaining before RB capital: keeper-fill probe trades + account setup + paper trading. Prior same-day: **VENUE DECISION LOCKED: GMTrade for client #1 "RB"** (owner decision; multi-venue doctrine — future clients select/specify venue, see `docs/STRATEGIC-DIRECTION.md`, commits e2fc13a + a8392aa; full ranking: GMTrade 10/10, HL-direct 10/10 fallback, Jupiter 6/10, Phantom UI 6/10). Prior same-day: Flash Trade wind-down response COMPLETE — trader HALTED, all funds EXTRACTED. Flash announced winding down (X post, Aug 7). Response executed in one session: (1) trader halted via `RTP_TRADER_DRY_RUN=1` (deploy `51add6e1`, SUCCESS; verified FLAT on-chain via position PDAs), (2) deposit ledger drained — **API path (`/transaction-builder/withdraw`) paid only 25% of the SOL entry (0.375571401 wSOL) then settled zero on every retry; the UI path (Phantom import → flash.trade) withdrew the remaining USDC 76.59 + JitoSOL 0.032 instantly and in full**, (3) wSOL unwrapped to native SOL. Final wallet state: native SOL 1.2757, USDC 76.59, JitoSOL 0.032 ≈ 1.45 SOL equiv recovered of 1.5 deposited (gap ≈ lifetime trading fees). Key mechanics learned: withdrawal relayer vault `HRSohn83…` is hand-funded by the team (visible `DepositDirect` top-ups precede paid settles); the deposit-ledger SOL entry reads a stale book figure (1.5) that no longer corresponds to withdrawable value once other-mint entries are drained — zero settles are "nothing left," not errors. The ledger's `UserDepositLedger` PDA may keep showing the old SOL amount indefinitely; do not trust it as ground truth.
