================================================================================
  ...
  BUILD PLAN v3.2 — DASHBOARD DEPLOYED + SUBMISSION SPRINT
  Supersedes: BUILD_PLAN_v3.1
  Audit ref:  docs/SECURITY_AUDIT_2026-04-07.md
  Status:     ALL CRITICAL PATHS COMPLETE.
              Dashboard built (RTP Sentinel). GitHub Pages deploy in progress.
              Domain: resilientprotocol.xyz (Porkbun)
...
CONTEXT:
  ...
  (BUY→fill→SELL→fill→PnL round-trip verified), an autonomous devnet
  loop daemon on a 6h CI schedule with LLM-driven strategy evolution,
  and a live dashboard (RTP Sentinel) wired to the devnet treasury.
  This plan tracks the remaining deployment and submission work.
  ...
  DASHBOARD (DONE — pending GH Pages deploy):
  RTP Sentinel — Next.js dashboard showing live treasury SOL balance,
  swarm activity log, wing status, and constitutional invariants panel.
  Wired to devnet treasury PDA FNQbK1Vw77aT7qM1EMSmeEPDGizSNhX4rkkYBKQNFotF.
  Custom domain: resilientprotocol.xyz (DNS configured in Porkbun).
  ...
  Key links:
    ...
    Dashboard (live):    https://resilientprotocol.xyz
...
================================================================================
  ...
  [x] HL testnet API connected (207 assets)
  ...
  [x] RTP Sentinel dashboard built — Next.js, wired to devnet treasury
      - Live treasury SOL balance (PDA FNQbK1...otF)
      - Swarm activity log with Demo Replay
      - Wing status panel (Trading / Security / Evolve / Knowledge)
      - Constitutional Invariants panel (5 invariants visible)
      - DEVNET badge + Connect Phantom button
  [x] Custom domain purchased: resilientprotocol.xyz (Porkbun)
...
================================================================================
  DEMO COVERAGE STATUS (as of Apr 14)
================================================================================
  ...
  | 5. Observable treasury state | COVERED | Explorer link live. Devnet deposit TX confirmed. Dashboard live at resilientprotocol.xyz |
...
─────────────────────────────────────────────────────────────────────
  ...
  [x] LLM integration live on CI (used_llm: true confirmed)
  [x] 301 tests, 0 failures, 0 clippy warnings
  [x] RTP Sentinel dashboard built and running locally
  [x] Custom domain resilientprotocol.xyz purchased
...
WEEK 6 (Apr 14 – Apr 27): DEPLOY + POLISH + SUBMISSION
─────────────────────────────────────────────────────────────────────

  DASHBOARD DEPLOYMENT (IN PROGRESS)
  ─────────────────────────────────────────────────────────────────────
  Agent tasks (Claude/Cursor):
  [ ] Update dashboard/next.config.ts — add output: 'export', trailingSlash: true,
      images: { unoptimized: true }
  [ ] Add .github/workflows/deploy-dashboard.yml — GH Pages static export workflow
      with CNAME file injection (resilientprotocol.xyz)
  [ ] Audit dashboard/src/ for any next/image usage — ensure unoptimized is set
  [ ] Verify package.json build script outputs to dashboard/out/
  [ ] Any UI tweaks requested (minor cosmetic polish)

  Manual tasks (YOU):
  [ ] Porkbun DNS: add 4 x A records pointing to GitHub Pages IPs
        185.199.108.153 / .109.153 / .110.153 / .111.153
      and CNAME www -> tradewife.github.io
  [ ] GitHub repo Settings -> Pages -> Source: GitHub Actions
  [ ] GitHub repo Settings -> Pages -> Custom domain: resilientprotocol.xyz
  [ ] Tick "Enforce HTTPS" after DNS propagates (~5-10 min on Porkbun)
  [ ] Trigger first deploy: push to main or workflow_dispatch on deploy-dashboard.yml
  [ ] Verify https://resilientprotocol.xyz loads dashboard

  SUBMISSION SPRINT
  ─────────────────────────────────────────────────────────────────────
  [ ] Register individually on Colosseum BEFORE MAY 4: https://arena.colosseum.org
      *** HARD DEADLINE — CALENDAR THIS NOW ***
  [ ] Demo rehearsal — run 3-minute script end-to-end, verify all 5 judge points
  [ ] Record demo video (2-3 min) — covers: dashboard live, devnet loop CI run,
      demo.rs 8-step pipeline, constitutional invariant rejection on-chain
  [ ] Final security sweep of public-facing files
  [ ] Submit to Colosseum by May 11

  DELIVERABLES:
    [x] Trading Wing places real order on HL testnet
    [x] demo.rs shows two-cycle memory persistence + heartbeat redirect
    [x] Treasury state visible (explorer link + deposit TX)
    [x] Devnet loop running autonomously with LLM-driven evolution
    [ ] Dashboard live at https://resilientprotocol.xyz
    [ ] Demo video recorded
...
================================================================================
  ...
  |---|--------------------------------|------------|-------|
  | 1 | PDA owns treasury              | DONE       | — |
  | 2 | TransferFeeConfig immutable    | DONE       | M-1 fixed — verified in initialize |
  | 3 | CPI-only transfers             | DONE       | — |
  | 4 | Agent proposes, human approves | DONE       | H-1 + C-1 fixed |
  | 5 | No SOL liquidation             | DONE       | HL positions are USDC-margined |
  | 6 | Phase transitions irreversible | DONE       | — |
  | 7 | Soulcontract amend = human sig | STUB       | Production: ed25519 on reload(). Demo unaffected. |
  | 8 | Auto-rollback >5% degradation  | DONE       | H-5 fixed — reads from spec |
  | 9 | Self-hydration >90-day runway  | DONE       | — |
...
================================================================================
  ...
  | GH Pages deploy fails (Next.js SSR leak) | LOW | LOW | output: 'export' enforced; unoptimized images required |
  | DNS propagation delay | LOW | LOW | Porkbun typically <10 min; explorer link is fallback |
