...
| **Execution** | On-chain `open_flash_position` re-checks 20% cap, runway floor, max-3 concurrent | Capital deployment |
...
- **No SOL liquidation:** positions are on-chain Solana perps via Flash Trade CPI. SOL never leaves the chain. No cross-chain bridge.
