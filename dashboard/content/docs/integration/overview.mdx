...
For launchpads that prefer not to run chain operations themselves, RTP will offer an Enterprise API that handles all on-chain interactions via REST:

```
Launchpad ──POST /v1/adopt──▸ RTP Enterprise API ──▸ Solana
                              │
...
> **Planned Feature** — The Enterprise API is in development. See the [Enterprise API Reference](/docs/integration/mpc-api) for the planned endpoint documentation.
